...
// swift-vim-engine v2.0 marks VimEngine and VimTextEditor as
// @MainActor. Top-level executable code is statically nonisolated
// (even though it runs on main at runtime), so the calls have to
// be made from within a MainActor-isolated context. assumeIsolated
// is the lightweight way to acquire that context without
// restructuring into an `@main` struct.
MainActor.assumeIsolated {
    ...
    @MainActor func expect(_ condition: Bool, _ message: String = "", file: String = #file, line: Int = #line) {
        if condition {
            passed += 1
        } else {
            failed += 1
    ...
    @MainActor func makeTempDir() -> URL {
    ...
    @MainActor func makeRegistry(seeded: Bool = true) -> (SlashCommandRegistry, URL) {
        let dir = makeTempDir()
        let store = TemplateStore(directoryOverride: dir)
    ...
    @MainActor final class StubEditor: VimTextEditor {
        var text: String
    ...
    @MainActor func feed(_ engine: VimEngine, _ keys: [String], on editor: VimTextEditor) {
        for k in keys {
